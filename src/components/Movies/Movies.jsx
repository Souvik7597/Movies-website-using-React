...
export default function Movies() {
    ...
    useEffect(() => {
        ...
        const fetchMovies = async () => {
            const response = await fetch(`${API_BASE_URL}/movie/now_playing?page=${page}`, options)
        ...
        <div className="container-width py-12">
            <header className="flex items-center justify-between relative">
                <ul className="text-xl uppercase font-semibold items-center gap-5 hidden md:flex">
                    ...
                    {
                        moviesLink.map((movielink) => (
                            <Link key={movielink.id} to={movielink.links}>
                                <li className="text-white xl:hover:text-[#ffe400] transition-colors duration-300">{movielink.nav}</li>
                            </Link>
                        ))
                    }
...

                    {
                        ...
                        <ul className="z-50 text-lg uppercase font-semibold flex flex-col justify-center items-center gap-5 bg-white px-8 py-5 rounded-md absolute top-12 left-0">

                            {
                                moviesLink.map((movielink) => (
                                    <Link key={movielink.id} to={movielink.links}>
                                        <li className="text-black xl:hover:text-[#ffe400] transition-colors duration-300">{movielink.nav}</li>
                                    </Link>
                                ))
                            }
